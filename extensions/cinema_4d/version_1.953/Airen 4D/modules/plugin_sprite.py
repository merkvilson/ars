...
class OAI_SPRITE(c4d.plugins.ObjectData):

    def __init__(self) :

        self.bmp = c4d.bitmaps.BaseBitmap()
        self.img_dir = sd_sprite_dir

        self.polygon = c4d.BaseObject(5174)
        self.polygon[c4d.PRIM_AXIS]=5


    def Init(self, node):

        apply_defaults(node, "sprite")
    ...
    def GetDDescription(self, node, description, flags):
        ...
        if not description.SetParameter(settings_tab, bc, c4d.DescID(c4d.DescLevel((node.GetType())))):return False
    ...
    def Message(self, node, type, data):
        ...
        if type == c4d.MSG_DESCRIPTION_CHECKUPDATE:
            ...
            thumbnail_dropdown_msg(desc_id, node, self.img_dir)


            if desc_id in [RNDR_PREV_SEED,RNDR_CRNT_SEED,RNDR_NEXT_SEED,]:

                render_simple_prompt(node, "rembg.json", "sprites/sprite")
